...
try
  ...
  if not DirectoryExists(TPath.Combine(IncludeTrailingPathDelimiter(SysUtils.GetEnvironmentVariable('APPDATA')), name)) then
    CreateDir(TPath.Combine(IncludeTrailingPathDelimiter(SysUtils.GetEnvironmentVariable('APPDATA')), name));

  DirPath := TPath.Combine(IncludeTrailingPathDelimiter(SysUtils.GetEnvironmentVariable('APPDATA')), name);

  CoinFilePath := TPath.Combine(IncludeTrailingPathDelimiter(SysUtils.GetEnvironmentVariable('APPDATA')), name);
  CoinFilePath := TPath.Combine(CoinFilePath, 'hodler.coin.dat');

  TokenFilePath := TPath.Combine(IncludeTrailingPathDelimiter(SysUtils.GetEnvironmentVariable('APPDATA')), name);
  TokenFilePath := TPath.Combine(TokenFilePath, 'hodler.erc20.dat');

  SeedFilePath := TPath.Combine(IncludeTrailingPathDelimiter(SysUtils.GetEnvironmentVariable('APPDATA')), name);
